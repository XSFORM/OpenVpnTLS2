...
function installOpenVPN() {
	...
	if [[ ! -d /etc/openvpn/easy-rsa/ ]]; then
		...
		./easyrsa --batch build-ca nopass
                # Автоматически снять пароль с приватного ключа CA, если он есть
CA_KEY="/etc/openvpn/easy-rsa/pki/private/ca.key"
if [[ -f "$CA_KEY" ]] && grep -q "ENCRYPTED" "$CA_KEY"; then
    echo "CA private key is encrypted. Removing password..."
    # Попробуем EC-ключ
    if grep -q "EC PRIVATE KEY" "$CA_KEY"; then
        # Запросим пароль у пользователя один раз для расшифровки
        openssl ec -in "$CA_KEY" -out "${CA_KEY}.nopass" && {
            mv "$CA_KEY" "${CA_KEY}.encrypted"
            mv "${CA_KEY}.nopass" "$CA_KEY"
            chmod 600 "$CA_KEY"
            echo "Пароль успешно снят с EC CA ключа."
        }
    # Попробуем RSA-ключ
    elif grep -q "RSA PRIVATE KEY" "$CA_KEY"; then
        openssl rsa -in "$CA_KEY" -out "${CA_KEY}.nopass" && {
            mv "$CA_KEY" "${CA_KEY}.encrypted"
            mv "${CA_KEY}.nopass" "$CA_KEY"
            chmod 600 "$CA_KEY"
            echo "Пароль успешно снят с RSA CA ключа."
        }
    else
        echo "Неизвестный тип ключа. Не удалось автоматически снять пароль."
    fi
fi    
